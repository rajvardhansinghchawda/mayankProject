...
const TestCard = ({ test }) => {
  const getStatusColor = (status) => {
    switch (status) {
      case 'Active': return 'bg-primary text-white shadow-lg shadow-primary/20 animate-pulse';
      case 'Upcoming': return 'bg-tertiary-fixed text-on-tertiary-fixed-variant';
    ...
    <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-[0_12px_32px_rgba(25,28,29,0.04)] hover:shadow-[0_20px_40px_rgba(25,28,29,0.08)] transition-all group flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-4">
          <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full ${getStatusColor(test.status)}`}>
            {test.status}
          </span>
        ...
        <p className="text-xs font-semibold text-slate-400 mb-6 uppercase tracking-wider">{test.subject}</p>
        ...
        <div className="space-y-3 mb-8">
          ...
          <div className="flex items-center gap-3 text-on-surface-variant">
            ...
            <span className="text-sm font-medium">{test.duration} • {test.questions} Questions</span>
          </div>
          <div className="flex items-center gap-3 text-on-surface-variant">
            <span className="material-symbols-outlined text-[18px]">description</span>
            <span className="text-sm font-medium">{test.type} • {test.points} Marks</span>
          </div>
        </div>
      </div>

      <Link 
        to={test.status === 'Active' ? '/assessments/instructions' : '#'}
